...
브라우저가 열리고, 새로 들어온 제보를 사진과 함께 하나씩 봅니다.
...
def gather(workdir):
    rows = sql("SELECT id, round, region, detail, email, sido, sigungu, photos, "
               "created_at FROM reports WHERE status = 'new' ORDER BY created_at")
    if not rows:
        return []
...
def build_html(rows):
    ...
    for row in rows:
        shots = ''.join(
            ...
            for f in row['files'])
        picks = ''.join(
            '<label class="pick"><input type="radio" name="s{id}" value="{v}">'
...
<header class="top">
  <h1>제보 검토실</h1>
  <span class="left">새 제보 <b id="left">%COUNT%</b>건 · 고른 것만 반영됩니다</span>
...
</html>""".replace('%CARDS%', ''.join(cards) or
                   '<p class="empty">검토할 새 제보가 없습니다.</p>') \
...
def main():
    ...
    try:
        rows = gather(workdir)
        if not rows:
            print('검토할 새 제보가 없습니다.')
            return

        print('새 제보 %d건. 브라우저에서 검토하세요 — http://127.0.0.1:%d/'
              % (len(rows), PORT))
