...
> ⚠️ 지금 데이터는 2018년 기준이라 이후 행정구역 변경이 반영돼 있지 않다.
> 새 경계 데이터를 받으면 이 스크립트를 다시 돌리면 된다.
>
> ⚠️⚠️ **이름 둘은 2026-09-08 에 손으로 고쳤다** — `강원특별자치도`·`전북특별자치도`.
> **이 스크립트를 다시 돌리면 2018년 이름으로 되돌아간다.** 돌린 뒤에는
> `public/data/map-sido.json` 과 `map-sigungu/32.json`·`35.json` 의 이름을
> 다시 고칠 것. (경계선 자체는 이름만 바뀐 것이라 달라지지 않았다.)
